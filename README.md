...
首次运行时会：
- 为所选服务创建 Python venv / 安装 `node_modules`（只装缺失或不可用的那一侧）
- 首次调用识别接口时自动下载 ONNX 模型（几十 MB，需要网络）

### 跨平台启动器（推荐）

`start.mjs` 是唯一启动逻辑，支持从任意目录调用。`backend`、`frontend`、`label-tool` 都是一等服务：

```bash
node /path/to/ZMD/start.mjs backend
node /path/to/ZMD/start.mjs frontend
node /path/to/ZMD/start.mjs label-tool
node /path/to/ZMD/start.mjs backend frontend
node /path/to/ZMD/start.mjs frontend label-tool
node /path/to/ZMD/start.mjs backend label-tool
node /path/to/ZMD/start.mjs backend frontend label-tool
node /path/to/ZMD/start.mjs all
node /path/to/ZMD/start.mjs
```

不传参数等同于 `all`，会启动三项：
- 后端 OpenAPI：http://localhost:8000/docs
- 前端：http://localhost:5173
- 模板标注工具：http://localhost:5174

日志会带 `[backend]` / `[frontend]` / `[label-tool]` 前缀并按色区分。Ctrl+C 会按反向启动顺序清理所有子进程。

### 平台专属脚本（可选）

`start.sh` / `start.ps1` 是薄壳子，会定位仓库目录后转发参数给 `start.mjs`。

macOS / Linux：

```bash
/path/to/ZMD/start.sh backend
/path/to/ZMD/start.sh frontend
/path/to/ZMD/start.sh label-tool
/path/to/ZMD/start.sh backend frontend
/path/to/ZMD/start.sh frontend label-tool
/path/to/ZMD/start.sh backend label-tool
/path/to/ZMD/start.sh backend frontend label-tool
/path/to/ZMD/start.sh all
/path/to/ZMD/start.sh
```

Windows（最简：双击 `start.cmd`）：

`start.cmd` 是给"双击就跑"准备的。直接在文件浏览器里**双击它**会自动开命令行、装依赖、起服务，三个端口都默认拉起来。要单独起某个服务也可以从 cmd 里调：

```cmd
start.cmd backend
start.cmd frontend
start.cmd label-tool
start.cmd backend frontend
start.cmd
```

> 为什么不双击 `start.ps1`：Windows 默认双击 .ps1 会打开 Notepad 编辑而不是执行。要么用下面的 PowerShell 调用方式，要么直接双击 `start.cmd`。
...
.\start.ps1 backend
.\start.ps1 frontend
.\start.ps1 label-tool
.\start.ps1 backend frontend
.\start.ps1 frontend label-tool
.\start.ps1 backend label-tool
.\start.ps1 backend frontend label-tool
.\start.ps1 all
.\start.ps1
```
...
├── start.sh               # macOS / Linux 启动薄壳
├── start.cmd              # Windows 双击启动器（cmd 薄壳，新手友好）
├── start.ps1              # Windows PowerShell 启动薄壳
├── start.mjs              # 跨平台服务启动器
