...
- **游戏数据**（材料 / 干员 / 武器 / 升级成本表）：[end.wiki](https://end.wiki/zh-Hans/)，通过 `scripts/port-from-endwiki.mjs` 自动抓取生成
- **图片素材**（干员头像 / 武器图标 / 材料图标）：`cdn.end.wiki`，WebP 抓取后转 PNG
- **基质规划数据**（能量淤积点副/技能词池 + 武器理想词条）参考：[Arknights-yituliu/ef-frontend-v1](https://github.com/Arknights-yituliu/ef-frontend-v1)（GPL-3.0，仅复用游戏机制数据，算法与 UI 本项目独立重写）
