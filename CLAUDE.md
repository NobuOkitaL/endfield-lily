...
- **Multi-crop OCR** — operators uses 4 fracs `(0.70, 0.60, 0.50, 0.40)` with max-digits voting. **inventory and weapons** use 6 fracs `(0.85, 0.78, 0.70, 0.60, 0.50, 0.40)` plus `_prepare_qty_ocr_image` / `_prepare_weapon_level_ocr_image` (3× upscale + sharpen + Otsu binarize) on each crop; voting groups all parsed values across raw+preprocessed × all fracs and picks `(occurrence_count, digit_count, max_conf)` — icon silhouettes that masquerade as digits (e.g. red crystal spike on 燎石 reading as a leading "1") only appear once across variants while the legit reading repeats. Inventory also uses `eff_h = min(max(h, p75_height(slots)), canvas_h - y)` for the OCR region (not raw `h`)—edge_lattice augmented cells can be undersized (67×51 vs median 107×108) and would otherwise crop above the level text.
